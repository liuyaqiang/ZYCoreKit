...
#import "ZYFMDatabasePool.h"
    ...
    #define ZYFMDBAutorelease(__v) ([__v autorelease]);
    #define ZYFMDBReturnAutoreleased ZYFMDBAutorelease

    #define ZYFMDBRetain(__v) ([__v retain]);
    #define ZYFMDBReturnRetained ZYFMDBRetain

    #define ZYFMDBRelease(__v) ([__v release]);

    #define ZYFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define ZYFMDBAutorelease(__v)
    #define ZYFMDBReturnAutoreleased(__v) (__v)

    #define ZYFMDBRetain(__v)
    #define ZYFMDBReturnRetained(__v) (__v)

    #define ZYFMDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details.
    #if OS_OBJECT_USE_OBJC
        #define ZYFMDBDispatchQueueRelease(__v)
    #else
        #define ZYFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^ZYFMDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);


/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.

 ### Usage
 The three main classes in ZYFMDB are:

 - `ZYFMDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<ZYFMResultSet>` - Represents the results of executing a query on an `ZYFMDatabase`.
 - `<ZYFMDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also

 - `<ZYFMDatabasePool>` - A pool of `ZYFMDatabase` objects.
 - `<ZYFMStatement>` - A wrapper for `sqlite_stmt`.

 ### External links

 - [ZYFMDB on GitHub](https://github.com/ccgus/fmdb) including introductory documentation
 - [SQLite web site](http://sqlite.org/)
 - [ZYFMDB mailing list](http://groups.google.com/group/fmdb)
 - [SQLite FAQ](http://www.sqlite.org/faq.html)

 @warning Do not instantiate a single `ZYFMDatabase` object and use it across multiple threads. Instead, use `<ZYFMDatabaseQueue>`.

 */

@interface ZYFMDatabase : NSObject  {
...
/** Create a `ZYFMDatabase` object.

 An `ZYFMDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `ZYFMDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `ZYFMDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    ZYFMDatabase *db = [ZYFMDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    ZYFMDatabase *db     = [ZYFMDatabase databaseWithPath:dbPath];
 ...
 @return `ZYFMDatabase` object if successful; `nil` if failure.

 */

+ (instancetype)databaseWithPath:(NSString*)inPath;

/** Initialize a `ZYFMDatabase` object.
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(ZYFMDBExecuteStatementsCallbackBlock)block;
...
/** Execute select statement
 ...
 Executing queries returns an `<ZYFMResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.

 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[ZYFMResultSet next]>`) from one record to the other.
 ...
 @return A `<ZYFMResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.

 @see ZYFMResultSet
 @see [`ZYFMResultSet next`](<[ZYFMResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
 */

- (ZYFMResultSet *)executeQuery:(NSString*)sql, ...;
...
- (ZYFMResultSet *)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (ZYFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
- (ZYFMResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments;


// Documentation forthcoming.
- (ZYFMResultSet *)executeQuery:(NSString*)sql withVAList: (va_list)args;
...
+ (NSString*)ZYFMDBUserVersion;

+ (SInt32)ZYFMDBVersion;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    [queue inDatabase:^(ZYFMDatabase *adb) {
        ...
        int rowCount = 0;
        ZYFMResultSet *ars = [adb executeQuery:@"select * from ftest where StringStartsWithH(foo)"];
        while ([ars next]) {
            rowCount++;
            NSLog(@"Does %@ start with 'h'?", [rs stringForColumnIndex:0]);
        }
        ZYFMDBQuickCheck(rowCount == 2);
    }];
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [ZYFMDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter generated by this method should be assigned to only one ZYFMDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using ZYFMDatabase::storeableDateFormat.
 ...
 @warning Note there is no direct getter for the `NSDateFormatter`, and you should not use the formatter you pass to ZYFMDB for other purposes, as `NSDateFormatter` is not thread-safe.
 */
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using ZYFMDB you will not need to interact directly with `ZYFMStatement`, but rather with `<ZYFMDatabase>` and `<ZYFMResultSet>` only.
...
@interface ZYFMStatement : NSObject {
