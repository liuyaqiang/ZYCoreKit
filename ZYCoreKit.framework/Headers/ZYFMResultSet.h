...
/** Represents the results of executing a query on an `<ZYFMDatabase>`.
...
@interface ZYFMResultSet : NSObject {
    ZYFMDatabase          *_parentDB;
    ZYFMStatement         *_statement;
...
/** `ZYFMStatement` used by result set. */

@property (atomic, retain) ZYFMStatement *statement;

///------------------------------------
/// @name Creating and closing database
///------------------------------------

/** Create result set from `<ZYFMStatement>`

 @param statement A `<ZYFMStatement>` to be performed

 @param aDB A `<ZYFMDatabase>` to be used

 @return A `ZYFMResultSet` on success; `nil` on failure
 */

+ (instancetype)resultSetWithStatement:(ZYFMStatement *)statement usingParentDatabase:(ZYFMDatabase*)aDB;

/** Close result set */

- (void)close;

- (void)setParentDB:(ZYFMDatabase *)newDb;
